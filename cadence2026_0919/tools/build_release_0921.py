#!/usr/bin/env python3
"""Freeze the camera-ready evidence into release stubs (grapemots-protocol r27).
...
# 2026-09-21: the robustness round (release r27)
FAMILIES.update({
    "bytetrack": sorted(glob.glob("runs/rev_0920/bytetrack/*_s*_d*.json")),
    "strongsort": sorted(glob.glob("runs/rev_0920/strongsort/*_s*_d*.json")),
    "retrain": sorted(glob.glob("runs/rev_0920/retrain/*/*_s*_d*.json")),
    "apple_val0000": sorted(glob.glob("runs/rev_0920/apple_val0000/apple_s*_d*.json")),
})
    ...
    for fam, paths in FAMILIES.items():
        ...
        with tarfile.open(tpath, "w:gz", compresslevel=9) as tar:
            for p in paths:
                ...
                name = f"{fam}/{os.path.basename(os.path.dirname(p))}__{os.path.basename(p)}" if fam == "retrain" else f"{fam}/{os.path.basename(p)}"
