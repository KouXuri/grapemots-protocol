...
def main() -> None:
    ...
    rows = [
        ...
        ("annotated", ANNOT, INK, "o"),
        ("interval 1", ANNOT, ACCENT, "o"),
        ("interval 4, phase 0", ANNOT[0::4], ACCENT, "o"),
        ("interval 4, phase 1", ANNOT[1::4], ACCENT, "o"),
    ]
    for k, (label, idx, colour, marker) in enumerate(rows):
        ...
        ax.text(-1.2, y, label, ha="right", va="center", fontsize=8.0, color=INK)
    ax.text(N + 0.6, 3.0, "reference G:\nall of\nthis row", ha="left", va="center",
            fontsize=8.0, color=INK, linespacing=1.1)
    ax.annotate("", xy=(N + 0.3, 2.35), xytext=(N + 0.3, -0.35),
                arrowprops=dict(arrowstyle="-", color=ACCENT, lw=1.0))
    ax.text(N + 0.8, 1.0, "processed:\nwhat the\ntracker sees", ha="left",
            va="center", fontsize=8.0, color=ACCENT, linespacing=1.1)
    ax.set_xlim(-10.5, N + 9.5)
